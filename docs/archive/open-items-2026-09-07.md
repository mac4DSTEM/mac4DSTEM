# Open items as they stood before the C1 trim — 2026-09-07 (verbatim)

`docs/open-items.md` exactly as it was before the consolidation plan's C1
session trimmed every entry to the file's own ≤ 12-line rule, dated the
undated, fixed six stale line pins, archived four closed entries to
`closed-items-2026-09.md`, reopened the `.icns` item, and moved "Working
methods that earned their keep" into `docs/development-process.md`. Nothing
below is lost; nothing below is current. **History, not guidance.**
