...
Every quantity carries its calibration and its provenance through display, export and reopening. Where a measurement is not quantitatively supported, theresult is labelled accordingly rather than presented as though it were.

## Status
**v1.0.0** — first release. The analysis workflow is complete and covered by 30 automated test suites, including parity harnesses measured against py4DSTEM 0.14.19 on experimental datasets.
Stated precisely: that verification is numerical. The application is tested against known-correct values, not against its rendered output. The Merlin MIB and EMPAD RAW readers remain preview-grade pending further vendor data. Current limitations are documented in [`docs/open-items.md`](docs/open-items.md).
