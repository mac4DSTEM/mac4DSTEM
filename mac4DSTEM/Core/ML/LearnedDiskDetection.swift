...
//  Role: The learned candidate stage's disagreement diagnostics (any platform)
//        and its full-scan streaming orchestration (macOS 27+, Core AI) —
//        docs/v3-plan.md §3a, step 4 slice 2.
//
...
// MARK: - Streamed full-scan orchestration (Core AI, macOS 27+)
...
extension LearnedDiskDetector {
    ...
    /// The pinned asset shipped in the app bundle
    /// (Contents/Resources/DiskDetector/disk-detector-heatmap-b32.aimodel), or nil.
    package static func bundledAssetURL(in bundle: Bundle = .main) -> URL? {
        bundle.url(forResource: "disk-detector-heatmap-b32", withExtension: "aimodel", subdirectory: "DiskDetector")
            ?? bundle.url(forResource: "disk-detector-heatmap-b32", withExtension: "aimodel")
    }
}
#endif
