...
| 7 Phase split (DPC & iDPC / Parallax / Single-slice ptychography), revisitable stages, inspector follows the task, chip slider | 7a, 7b, 7d done 2026-09-03; 7c slices 1–3 and 4a done 2026-09-02 | **7c 4b, 5, 6 next** (plan §11h, one commit each). Landed: `ResultsSidebar` (the saved-product chooser, out of the detail pane), `ProductInspector`, `PrepareSidebar`, `ImageSidebar`, `MapSidebar`; `FocusedPane` on `WorkspaceNavigation` — every pane but Phase's claims it and the inspector's Live group renders from it (`nil` = the 7b per-task conditions, an adapter that expires at slice 5); no UI file reads a calibration forwarder. **4b owed:** the ACOM/strain run functions into their sessions, the ACOM forwarder block deleted — see the owner note |
...
| Gate | Result |
|---|---|
| `run-tests.sh unit` | 461 passed / 0 failed / 3 skipped, exit 0 — 2026-09-02, 7c slice 4a tree (skips: unmounted-volume probe, S17 quarantine, `TB1StallProbeTests` fixture absent) |
...
- **Start here — 7c slice 5 (Phase), then 6:** `docs/v2.5-plan.md` §11h
  lists the slices; 1–3 and 4a landed, 4b waits on the owner note below.
  `PhaseSidebar` takes the DPC, parallax and ptychography sections (and the
  `reconstructionStage` helpers) out of `ContentView`; Phase's panes claim
  `FocusedPane` and the `AppState.inspectorShows*` adapter is deleted; the
  calibration forwarder block goes once `AppState`'s own reads go through
  `calibrationSession` (watch for locals named `calibration` — a blind
  rewrite would silently read the session instead). Then `ContentView` as
