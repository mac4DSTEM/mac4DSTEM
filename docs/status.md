...
| Gate | Result |
|---|---|
| `run-tests.sh unit` | **457 passed / 0 failed / 1 skipped, exit 0 — 2026-09-04, `ActivityLog` tree** (458 tests). Full log retained; the exit code is read on its own line, never through a `tail` pipe, and the tests are counted by method name and then reconciled against the expected delta (`open-items.md`, working methods — a chopped log line has now faked a missing test twice). Same day, same discipline: 455/0/1 (status bar, +4), 448/0/1 (dead focus model, −9 +2), 451/0/1 (path leak, +3) |
