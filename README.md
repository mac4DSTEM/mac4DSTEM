...
- An **Advanced Detection** disclosure exposes the py4DSTEM controls with live current-CBED feedback: separate diffraction/correlation Gaussian widths, absolute and relative intensity cutoffs plus reference-peak rank, minimum spacing, edge exclusion, maximum peaks, and `multicorr` upsampling. Changing settings after a full-scan pass is marked stale until detection is rerun, so strain/ACOM inputs are not misrepresented.
