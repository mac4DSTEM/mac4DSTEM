...
| F1.3b | **FAILED 2026-08-18 — mis-scored PASSED earlier the same day, corrected** | Press **Open with Options…**, pick a large dataset | The configurator appears with both previews, a stride line, a bin picker and the size table. **"Open Dataset…" must behave exactly as before** — no configurator, no extra step | The opt-in entry, the stride line, the bin picker and the size table are all correct, and plain open is untouched. **But neither preview image draws.** Both panes are empty at their full 220pt height — see the finding in `docs/open-items.md`. The row was first scored PASSED on the dialog *appearing*, which is exactly the mistake Track B exists to prevent: the labels were read and the image area was not |
