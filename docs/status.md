...
table's first two rows); the owner drove the picker and the map's Provenance rows on
the demo at 13:03–13:06 (the session-2 row); nothing from C7 is unseen. C6's evidence is `archive/v3/learned-detector-2026-09-06.md` "C6 — the
...
- Nothing on screen from C7 (session 2 driven 2026-09-08); C4 slices 1–2 remain undriven. The push of `ml/disk-detector` `f057545` and of `main`.
