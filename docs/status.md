...
| Gate | Result |
|---|---|
| `run-tests.sh unit` | **451 passed / 0 failed / 1 skipped, exit 0 — 2026-09-04, path-leak tree** (452 tests). Full log retained; the exit code is read on its own line, never through a `tail` pipe, and the tests are counted by unique NAME, never by grepping lines (`open-items.md`, working methods). Earlier the same day, same discipline: 455/0/1 (status bar, +4), then 448/0/1 (the dead focus model: −9, +2). Before the UI retirement it was 476/0/3; that commit deleted 27 AppKit-shell tests |
