...
| F1.16 | **UNVERIFIED — queued by S4, 2026-08-24** | **Open with Options…** on a real cube. Three panes should draw: real space, max DP, and **single position**. **Click** (don't drag) the real-space preview on a recognisable feature near its right edge | All three panes show real structure (F1.3b's bar); the single-position pane changes to that position's actual pattern and its caption names the clicked **source** scan position — for a click near the right edge, a column near the full scan width | The caption names the position so the stride trap is visible: the preview is on the sampled grid and the click is multiplied by the stride, so a missing multiplication would name a column at a *fraction* of where you clicked. Also check a plain click did not start a crop — drag and click must stay separate gestures |
| F1.17 | **UNVERIFIED — queued by S4, 2026-08-24** | Read the configurator's copy and size table | The dialog states **Scan (Ry x Rx)** and **Detector (Qy x Qx)** matching the inspector's *Dimensions* rows for the same cube; a caption explains that a **crop cuts angular range** while a **bin coarsens sampling** (costing disk-position precision); and the sheet shows all its sections **without scrolling or clipped headers** at the new 900×760 size | The two-orders trap: the sidebar header prints the scan the other way round (S18's item) — this row only requires the dialog to match the *inspector*, not the sidebar |
| F1.18 | **UNVERIFIED — queued by S4, 2026-08-24** | In the configurator, drag a detector crop into one **corner**, away from the bright centre | An orange refusal appears naming the beam's detector column/row and the crop's extent, and **Load disables**. Widen the crop over the beam: the refusal clears and Load enables | This replaces F1.7's configurator scenario: the refusal now fires **before** the load, at configure time. The beam evidence is the sampled preview's brightest region, so on a beam-stopped acquisition this gate could refuse a legitimate crop — if that ever fires wrongly, that is a finding to record, not a reason to widen the gate in place |
