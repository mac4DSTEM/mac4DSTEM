...
| S2 | A · T1 OR | T1 relationship derived into the OR form; re-measure; unit + `phase-vector-matching` harness | D + B | Sonnet derivation, Fable ran it | done 2026-09-21: edge-on → T1 295 → 45, Al → T1 264 → 176, T1 recall 99 % kept; edge-on now refuses (250) — `archive/v3/t1-relationship-2026-09-21.md` |
| S3 | A · wire | Classifier picker in Phase mapping settings; class map → objects, per-class fraction/density in Results; export; badge logic | unit + inventory; **on-screen owed** | Sonnet | done 2026-09-21 (7 tests, unit 802/0), unverified on screen; θ′ measured → per-phase slab slice follows |
