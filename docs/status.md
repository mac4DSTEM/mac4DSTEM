...
| Gate | Result |
|---|---|
| `run-tests.sh unit` | **exit 0 — 2026-09-08, the C7 session-2 tree** (`scratchpad/unit-c7s2-20260908.log`, `GATE_EXIT=0` on its own line): **534 passed / 0 failed / 1 skipped, 535 cases** (535 `func test` in source). The script's own `xcodebuild test` line run directly because the 8 GB preflight refuses at < 2 GB free. Earlier: 514/0/1 on the session-1 tree (`unit-c7-20260908.log`). |
...
| `run-tests.sh core` (both packages) | **exit 0 — 2026-09-08, the C7 session-2 tree** with `Session/LearnedDetection.swift` (`scratchpad/core-c7s2-20260908.log`). Previous: the session-1 tree, same day |
| `run-tests.sh inventory` | **exit 0 — 2026-09-08, the C7 session-2 tree** (`scratchpad/inventory-c7s2-20260908.log`): gated 46, diagnostic 9; `AppState` + `ResultExport` **7 530** (7 531 at HEAD); live markdown **4 781**, up 23 from 4 758 — the session row and an append-only decisions entry; the cold-start set fell 884 → 873 — the session-2 row and its decisions; the reason is stated here. Previous: exit 0 on 2026-09-07 night (`inventory-owner-20260907.log`), gated 45, live markdown 4 677, cold-start 884. |
...
of its §6 whose exit criterion fails — **next for an agent: C7 session 3 (the
position-matched disagreement map, labels in the sidecar, the scan-bench
ported to Core ML and the 2.81× ceiling re-measured at 256 px; then the
Gate B campaign at C7's end); then C4 (b), staleness (item 1's C4 paragraph).**
...
2026-09-08: sessions 1 and 2 put the runtime and the picker on `main`** (the
table's first two rows); the owner drove the picker on the demo at 13:03 (the
session-2 row); the map's Provenance rows after a neural-net run are unseen. C6's evidence is `archive/v3/learned-detector-2026-09-06.md` "C6 — the
...
- On screen, unseen: the Bragg vector map's Provenance rows after a neural-net run (C7 session 2; the picker itself was driven 2026-09-08). The push of `ml/disk-detector` `f057545` and of `main`.
