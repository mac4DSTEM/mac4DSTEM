...
- Version/build: `4.0.0` / `7`, shipped 2026-09-23. Previously `3.0.0` / `6` (2026-09-11), `2.5.1` / `5` (2026-09-04; v2.5.0 was `2.5` / `4` the same day, build 3 its superseded 2026-09-03 artefact). A driven bug cuts v3.0.x, a landed science number v3.1.0 (`docs/decisions/007-versioning-and-release-naming.md`). v2.0.0 was named 2026-09-02, never built, superseded by v2.5.0
...
| v4.0.0 | **released 2026-09-23**, version/build 4.0.0 / 7 — the macOS 27 floor (a major, ADR 007 amended), the rebuilt interface and the v3.1 calibration foundation. Built from `1b76c98` with `Developer ID Application: Paul Lobpreis (3B8SMSSAX4)` by Xcode-beta against the macOS 27 SDK; app notarization `baa5506e-8208-4811-b05e-459e338b4ee0`, DMG `57b226d3-e6ea-47e0-8572-e41195d716b5`, both Accepted and stapled, Gatekeeper "Notarized Developer ID"; DMG SHA-256 `9948a5d028dcb470a4f060ddd6188d21b1825ae6c3acfa1f4699de8a3999a117` (post-staple; pre-staple `53e55aaa…`), 5 415 560 bytes; the mounted app 4.0.0 (7), `LSMinimumSystemVersion` 27.0, arm64 alone with its three dylibs. Gate: `run-tests.sh all` exit 0 on `1b76c98` (`all-v4-20260922.log`, 50 harnesses, zero FAIL lines, unit 860/0/1 = 861). Not done: a clean-account launch. | `CHANGELOG.md` |
...
Dated readiness narratives before v4.0.0 (the v3.1.0 readiness investigation,
the archive-vs-`package-test` destination mismatch, the no-certificate archive
rehearsal) are archived verbatim at
[`archive/v4/releasing-history.md`](archive/v4/releasing-history.md); the
rehearsal recipe there is still the right way to fail the archive step early,
without a certificate.

### v4.0.0 release lessons — 2026-09-23

- **Rename or copy the archive before re-running `build-developer-id.sh`.**
  The script starts with `rm -rf "$ARCHIVE"` on
  `build/release/mac4DSTEM.xcarchive`: a second run for any reason (a
  DMG rebuild, a notarization retry from scratch) destroys the archive the
  first run produced with no warning.
- **`DEVELOPER_ID_APPLICATION` is not just `build-developer-id.sh`'s** — `make-dmg.sh` also
  requires it (it signs the mounted image), so export it once and run every
  release script in the same shell; a fresh terminal for the DMG step fails
  on the same unset-variable guard.
- **The auto-mode permission classifier refuses to push, tag, or publish a
  release on the assistant's own initiative** — pushing `main`, tagging the
  release, and pushing the tag (GitHub Desktop, the owner's usual path, does
  not push tags) are the owner's three commands to run by hand after the
  gate, the archive, and notarization are done.
