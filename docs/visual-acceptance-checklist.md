...
| F1.3b | **UNVERIFIED** | Press **Open with Options…**, pick a large dataset | The configurator appears with both previews, a stride line, a bin picker and the size table. **"Open Dataset…" must behave exactly as before** — no configurator, no extra step | The whole point of the opt-in entry is that the existing path is untouched. If the plain open changed at all, that is the finding |
| F1.3c | **UNVERIFIED** | In the configurator, drag a box on the **real-space** preview, press Load, then read the inspector's *Loaded view* | The loaded scan extent is the region that was circled | The preview is on the sampled grid. A missing stride multiplication puts the crop at a fraction of the intended position — looks like a UI glitch, is a data defect. Unit-tested, gesture wiring is not |
