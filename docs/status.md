...
table's first two rows); the Detector picker in Disk detection is on screen
and undriven — item 1 now has something to drive. C6's evidence is `archive/v3/learned-detector-2026-09-06.md` "C6 — the
...
- On screen, undriven: the Detector picker, its Threshold and Model rows and the map's provenance rows (C7 session 2). The push of `ml/disk-detector` `f057545` and of `main`.
