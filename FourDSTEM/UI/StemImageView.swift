//
//  StemImageView.swift
//  Role: The real-space result viewer (right pane). Shows whatever the current
//        analysis produced — a virtual-detector image, a DPC magnitude map, etc.
//        Magnify to zoom; click a pixel to make that the selected scan position
//        (which updates the diffraction pane on the left).
//
//  Click-to-select is the natural way to explore a 4DSTEM dataset: pick a point
//  in real space, inspect its diffraction. Drag-pan is intentionally omitted
//  here so a click is unambiguously "select", not "pan" (Stage-2 nicety).
//
...
struct StemImageView: View {
    ...
    /// (view y points down, same as image coordinates), brightness grows with
    /// magnitude → dark center.
    private var colorWheelLegend: some View {
        ...
        return ZStack {
            Circle().fill(AngularGradient(gradient: Gradient(stops: stops),
                                          center: .center))
            ...
            .gesture(
                SpatialTapGesture(coordinateSpace: .local)
                    .onEnded { value in
                        ...
                        app.moveScan(to: ScanPos(x: x, y: y))
                    }
            )
    }
    ...
    private var placeholder: some View {
        VStack(spacing: 8) {
            Image(systemName: "square.grid.3x3")
                ...
                .foregroundStyle(.secondary)
            Text(app.hasDataset
                 ? "Adjust the aperture or pick an analysis to generate an image"
