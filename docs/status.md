...
| 7 Phase split (DPC & iDPC / Parallax / Single-slice ptychography), revisitable stages, inspector follows the task, chip slider | 7a, 7b, 7d done 2026-09-03; 7c slices 1–3 done 2026-09-02 | **7c slices 4–6 next** (plan §11h, one commit each). Landed: `ResultsSidebar` (the saved-product chooser, out of the detail pane), `ProductInspector`, `PrepareSidebar`, `ImageSidebar`; `FocusedPane` on `WorkspaceNavigation` — Results, Prepare and Imaging panes claim it, the inspector's Live group renders from it, `nil` (Strain & ACOM, Phase) still means the 7b per-task conditions, an adapter that expires at slice 5; no UI file reads a calibration forwarder any more |
...
| Gate | Result |
|---|---|
| `run-tests.sh unit` | 460 passed / 0 failed / 3 skipped, exit 0 — 2026-09-02, 7c slice 3 tree (skips: unmounted-volume probe, S17 quarantine, `TB1StallProbeTests` fixture absent) |
...
- **Start here — 7c slice 4 (Strain & ACOM):** `docs/v2.5-plan.md` §11h lists
  the slices; 1–3 landed. `MapSidebar` takes the disks, strain and ACOM
  sections out of `ContentView`; its panes claim `FocusedPane`. Then the
  ACOM/strain run functions move into their sessions and the ACOM forwarder
  block goes (see the owner note below), Phase (the calibration forwarder
  block deleted once `AppState`'s own reads go through `calibrationSession`),
  `ContentView` as composition only. `/pickup` takes it from this row.
