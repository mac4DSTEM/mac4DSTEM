...
| `run-tests.sh unit` | **478 passed / 0 failed / 1 skipped, 479 cases, SUCCEEDED — 2026-09-05 evening, the origin kernel + the `realslices` refusal**, run through XcodeBuildMCP `test_macos` (`scratchpad/unit-mcp-20260905.json`) because `run-tests.sh unit` refused with exit 69 at 5 GB free against its 8 GB floor and `free-space.sh` found no clearable debris (`unit-origin2-20260905.log`). Earlier the same day: 478/0/1 via `run-tests.sh` on the kernel change alone (`unit-origin-20260905.log`), 478/0/1 for the Q-calibration estimator, 477/0/1 for the science lane, 472/0/1 after the UI-review fixes, 463/0/1 after the DM4 rework. |
| `run-tests.sh scientific` | **43 harnesses, exit 0 — 2026-09-05 evening**, `scratchpad/scientific-origin3-20260905.log`, `PYTHON=$HOME/miniconda3/envs/py4dstem/bin/python`. **It had been red since 02:09** (`datacube-discovery-test`: the `realslices` check committed in `3e0c1dd` against a reader that never refused it) **and since 12:55** (`peak-overlay-test` stopped compiling when `0a6f36b` made `radius` optional); the morning's green was quoted by five commits in between. Both fixed this evening; the first evening rerun stopped at each in turn (`scientific-origin-20260905.log`, `scientific-origin2-20260905.log`). |
| `run-tests.sh core` (both packages) | exit 0 — `b91f5bb`, 2026-09-03 |
| `run-tests.sh inventory` | exit 0 — 2026-09-05 evening, dirty tree (`scratchpad/inventory-20260905.log`): gated 45, diagnostic 9, cold-start set 981 (985 at the release closeout), live markdown 4 421. The clean-tree documentation check is skipped by design while this work is uncommitted. One new file, `tools/origin-fit-diagnostics/origin-kernel-twin.py`: the numpy twin behind §9's table, in the existing origin-diagnostics home. |
...
4. **The science lane, one item at a time.** Landed 2026-09-05, each
   ...
   the detection-threshold item (WS₂ finds only the beam at defaults),
