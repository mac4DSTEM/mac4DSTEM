...
**Stabilization state (2026-07-16):** The frozen v1 workflow is implemented. Current
work is limited to scientific-contract corrections, task-focused UI consolidation,
incremental architecture extraction, and distribution readiness. ACOM now requires an
explicit material model; tunable fallback Q sampling is permanently labelled
Exploratory, while physical output records the scale and its provenance. WS₂ is
explicitly unsupported until a validated lattice/basis/symmetry model is implemented.
...
The aggregate gate runs the complete native XCTest suite, 24 standalone scientific/
interoperability harnesses, the current real-data manifest, and the clean hardened
...
- A shared validated detection contract owns stable parameter identifiers, units, help text, detector-dependent bounds, defaults, and immutable provenance. The **Advanced Detection** disclosure projects that contract directly: separate diffraction/correlation Gaussian widths, absolute and relative intensity cutoffs plus reference-peak rank, minimum spacing, edge exclusion, maximum peaks, and `multicorr` upsampling. A live acceptance funnel shows which filter removes candidates; a full-scan summary flags sparse or ceiling-limited results. Changing settings after a full-scan pass is marked stale until detection is rerun, so strain/ACOM inputs are not misrepresented.
- Detected full-scan peaks export to a companion py4DSTEM 0.14 / EMD 1.0 `BraggVectors` `.h5`. The export carries the complete detection/kernel provenance alongside Q/R pixel calibration and `QR_flip`, converts detector axes at the format boundary, and atomically publishes a sidecar without modifying the source dataset.
...
- A validated `CrystalModel` contract keeps the lattice/metric, atomic basis, Lobato structure factors, declared point-group implementation, source, stable identifier, and provenance together. The current explicit catalog covers FCC/BCC/diamond materials and HCP magnesium; custom single-element cubic models use the same contract. Dataset names and pattern appearance never select a phase.
...
- *Limitations (v1):* physical matching needs calibrated Q sampling; without it, a separate tunable scale can run only an explicitly Exploratory result. Phase selection has no default. Validated point-group presentation covers cubic crystals plus HCP magnesium; phases outside that catalog, including WS₂, need a complete imported model and a validated point-group implementation before they can run. High-symmetry zone axes remain ambiguity-prone and should be judged by reliability and fit overlays.
...
The py4DSTEM-referenced core-analysis path (I/O → virtual imaging → calibration → DPC → disk detection → strain) is in place.

**Current focus:** stabilization, material-specific scientific validation, and public-release readiness. Developer ID signing/notarization and a clean-account smoke test remain credential-owner actions; see [`docs/releasing.md`](docs/releasing.md).

**Repository gate:** `tools/run-tests.sh all` is the only aggregate claim. It runs the native suite, all 24 portable scientific/interoperability harnesses, the checked-in real-data manifest, and hardened Release packaging. Machine-local ACOM benchmarking and native Accessibility smoke remain explicit extensions. MIB/EMPAD remain Preview until real vendor files are supplied.
...
- **Testing is deliberately layered.** The native suite covers production/workflow semantics, failure recovery, deterministic-demo contracts, and publication rendering. Twenty-four standalone scientific/interoperability harnesses plus the current real-data manifest and Release audit cover py4DSTEM parity, formats, forced tiles, cancellation, and distribution. The real ACOM benchmark and native UI smoke are explicit machine-local extensions; `tools/run-tests.sh all` remains the aggregate gate.
...
- Single-pattern disk detection has a dependency-free, source-locked parity harness (`tools/disk-detection-test/run.sh`). On a non-square 32×64 pattern it validates the synthetic sigmoid probe kernel, cross and hybrid correlation, pixel/parabolic/`multicorr` localization, detector-axis conversion, absolute/relative/edge/spacing/count filters, the acceptance diagnostics, and exported parameter provenance. Detector-adapted contract tests cover square and non-square Q grids, while the real-data acceptance manifest samples all four local cubes. The source-locked harness exposed and fixed refined peaks retaining their integer-pixel intensity instead of py4DSTEM's bilinear intensity.
...
- ACOM phase selection now resolves through a validated model catalog rather than a UI-specific material enum. Each model binds its cell, basis, symmetry, source, identity, and result provenance; no dataset-name inference or default phase is permitted. The matching harness validates every catalog entry and generates a plan with its declared symmetry. Non-cubic ACOM includes an HCP magnesium cell and the 12 proper rotations of 6/mmm; plan sampling, reduction, IPF-Z presentation, export provenance, and the UI key all retain the selected symmetry, while changing the model/custom cell invalidates stale plans.
...
- **Session schema v5** extends scientific metadata to lossless RGBA products as well as arbitrary scalar shapes. IPF-Z now retains independent row/column sampling and deterministic material/Q-scale/interpretation provenance through native and py4DSTEM reads. Parallax and ptychography products retain their source, depth/factor/filter, engine/method, iteration/error, and update controls while atomic rewrites preserve calibration, prior results, BraggVectors, source bytes, and opaque external nodes.
...
4. **Scientific validation is continuous** — quantitative iDPC, strain robustness, ellipse fitting, cubic/HCP ACOM, and current real-data operational checks are gated; orientation ground truth is not claimed for any phase without an explicit material-specific fixture.
...
- Treat `References/py4DSTEM-dev` as a versioned, read-only scientific source lock. Keep machine-local datasets outside Git and change production code in small, independently gated batches.
