...
- **Frozen shell** (owner, 2026-09-22): `UI/ContentView.swift`, `UI/WorkspaceView.swift`, `UI/WorkspaceInspector.swift`, `UI/LayoutPolicy.swift` and `App/WorkspaceNavigation.swift` change only against a picture the owner has accepted (ADR 035); every other UI session is a room, Prepare first.
