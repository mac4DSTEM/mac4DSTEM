...
Detail: `docs/archive/open-items-detail-2026-09-18.md`. **Seam 5 landed 2026-09-18:** `ResultPresentation` owns shared display, Bragg and virtual-detector presentation state; AppState.swift 3676 → 3240; owner drive on `sim_Au` passed. Simplicity debt: after seams 6–7, replace scattered manual result-version bumps with one standard invalidation API under the tests that pin today's exact order/count.
