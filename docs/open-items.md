...
**Still open:** row 4 (a shared harness helper, Gate B on it — a shared `fail` can green 46 harnesses at once); row 5 continues (the NEXT `AppState` seam, one per session) — **2026-09-18: the Calibration section moved out** to `App/AppState+Calibration.swift` and `App/AppState+PhaseContrast.swift` (AppState.swift 5476 → 4699 lines) at 17 `private`/`private(set)` widenings, all Parallax state: a placement seam, not encapsulation; **2026-09-18, overnight, seam 1: the state itself moved** into `Session/PhaseContrastProduct.swift` (`docs/appstate-seams-plan.md`), AppState.swift 4699 → 4656, net −11 widenings (all of step one's reversed), no forwarding properties; **seam 2, same night: ACOM leftovers moved into the existing `ACOMSession`** (6 properties/methods fully relocated, 7 stayed as AppState orchestration because they genuinely combine `descriptor`/`calibrationSession` with `acomSession` state — not forwarders), `calibrateQFromCrystal`/`generateOrientationPlan`/`runACOM`/`applyACOMDisplay` to `App/AppState+ACOM.swift`, AppState.swift 4656 → 4230, 3 widenings; **seam 3, same night: `DiskDetectionProduct`** — `diskParams` and its pure-param logic moved to the new owner (a `didSet`-turned-`onParamsChange` hook, on the `PhaseContrastProduct` precedent), the probe-kernel-generator and detection functions placed in `App/AppState+DiskDetection.swift` since they read AppState-only state (`probeKernel`, `braggVectors`, staying per the plan until seam 5); AppState.swift 4230 → 3819, 5 widenings (at the seam's cap); **seam 4, same night, the last unattended seam: `DPCProduct`** — `dpcDisplay` (didSet → `onDisplayChange` hook, same shape) moved to the new owner along with its `DPCDisplayMode` enum (needed for the standalone `DSTEMSession` package build); `dpcMilliradiansPerDetectorPixel` stayed on AppState (reads only `calibrationSession`, no dependency on the owner's own state); `runDPC`/`flipRotation180`/`applyDPCDisplay` to `App/AppState+DPC.swift`; `computeCoMField` untouched, as the plan specified. AppState.swift 3819 → 3676, 1 widening. **All four overnight seams (1–4) landed clean, every gate green, nothing pushed.** Seams 5–7 (display, dataset, load pipeline) are attended, owner-present sessions per the plan; rows 6–7 (`Support/ResultExport.swift` / `Core/Data/BraggVectorEMDWriter.swift` splits — see the "resultexport-split" and "braggvector-emd-writer-split" entries above, both started 2026-09-17); row 12 (the >1 000-line harness mains). **Row 9 is a do-not:** five different `median` bodies in Core stay separate until a Gate D shows they should agree (ADR 015).
Evidence and blast radii: `docs/archive/audit-2026-09-16/REPORT.md` §3.2. Owner: whoever picks a row.
Detail: `docs/archive/open-items-detail-2026-09-18.md`. **Seams 5–6 landed 2026-09-18:** `ResultPresentation` owns shared display, Bragg and virtual-detector presentation state; `DatasetSession` owns the live reader/array, dataset list, preview, loading state and stale-publish epoch; AppState.swift 3676 → 3054. Both owner drives passed, including seam 6's open/switch/reopen flow. Simplicity debt: after seam 7, replace scattered manual result-version bumps with one standard invalidation API under the tests that pin today's exact order/count.
