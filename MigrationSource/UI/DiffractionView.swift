...
//        currently-selected scan position, with optional log scaling (driven by
//        the toolbar), and overlays the interactive annular aperture when the
//        Virtual Detector mode is active.
//
//  NOTE ON ZOOM: this view is kept at 1:1 (no pan/zoom) so the SwiftUI aperture
//  overlay stays pixel-accurate against the GPU-rendered pattern. Coordinating
//  shader-space zoom with the overlay is a Stage-2 refinement; the real-space
//  result view (StemImageView) is where free zoom/pan lives.
//
    ...
    private var header: some View {
        HStack {
            Text("Diffraction (CBED)")
                .font(.headline)
            Spacer()
            // Mean/max need calibration (which computes them) to be useful;
            // until then the picker still works but shows the live pattern.
            if app.meanPattern != nil {
                Picker("", selection: $app.patternDisplayMode) {
    ...
    private func content(in size: CGSize) -> some View {
        if let pattern = app.displayedPattern {
            ...
            ZStack {
                ...
                // Detected Bragg disks for the current pattern.
