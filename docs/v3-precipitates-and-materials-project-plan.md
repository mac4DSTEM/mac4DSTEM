...
| S3 | A · wire | Classifier picker in Phase mapping settings; class map → objects, per-class fraction/density in Results; export; badge logic | unit + inventory; **on-screen owed** | Sonnet | next, after the θ′ edge-on reference is measured the way T1 was |
