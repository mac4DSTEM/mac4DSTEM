...
| `run-tests.sh inventory` | exit 0 — 2026-09-05 late evening, dirty tree (`scratchpad/inventory6-20260905.log`): gated 45, diagnostic 9, cold-start set 996 (977 after the detection-threshold commit; up by a plan entry for the learned-detector option and the bullseye item's landed/open split), live markdown 4 468. New files: `ProbeKernelFlatTests.swift`, three diagnostics in `tools/bragg-spacing-probe/` (the existing detection-diagnostic home). |
