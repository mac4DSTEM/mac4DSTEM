...
| Saved-session sidecar contents moved to the LEFT sidebar | done 2026-09-04, **rows agent-verified 2026-09-07** (`shots-c3/b3b-sidebar.png`, `b3d-reopened.png`: filename row, Calibration, Change…, Ignore… in the Session section, back after reopen); **the Remove context menu is still unopened** (no driver could raise a context menu — owner: one right-click) | `Section("Saved session sidecar")` is gone from `WorkspaceInspector`; the sidecar filename, Calibration, BraggVectors, the saved-result rows, Apply Saved Controls and Change…/Ignore… render in `WorkspaceSidebar`'s `Section("Session")`. Info keeps only the unreadable / does-not-fit sections — the explanation half of the split. Builds clean with no warnings and no test names the moved identifiers, which is also the limit of what any gate can say: this is placement, and only the owner's eyes close it. One judgement call to overrule or keep — Remove is now the result row's context menu, not a second visible row per result (`open-items.md`) |
...
(item 1's C4 paragraph); C3's four leftovers and C6's four steps are yours.** C0 closed and **C1 (docs truth) and C2
...
1. **Your drive. (OWNER ONLY.)** Nothing beyond Prepare, Imaging and Strain &
   ...
   40 screenshots):** everything the demo fixture and a 164 MB real file can
   show was seen; what is left needs your hands, about ten minutes: (1) light
   appearance (the launch-argument route produced no window), (2) right-click a
   Session row to find Remove, (3) a crop via "Open with Options…" from the
   empty-state view, then save → quit → reopen, (4) the one-peak warning text
   on a real cube. Findings from the drive are in `open-items.md`.
...
- **Ten minutes of your hands** (handoff item 1): light appearance, the Session row's Remove, a crop → save → quit → reopen, the one-peak warning text.
