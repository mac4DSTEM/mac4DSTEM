...
**A — the TOOLBAR reads as empty** (clarified with a labelled dummy: by "header" the owner meant the toolbar, not the breadcrumb row). He wants the file name, Save to Session, Reveal in Finder and the current run in the toolbar — the reverse of `window-design.md` §6.2. Proposed in a mock the same evening, Xcode's arrangement: the room's run button (Stop while busy) at the left beside the navigator toggle; the file name, or the live run as a display (name · bar · ETA, never a button — the 2026-09-04 "C…" trap), in the centre; Save, Reveal, dataset menu and inspector toggle at the right; the breadcrumb row would go. **Parked by the owner 2026-09-22: no UI decisions this session.** Frozen shell; nothing built.
**B — the bottom pane's tab controls (Output · Run · Lineage) "behave strange".** No detail yet; deferred by the owner. Residual from the strip finding: the "second workspace" is phase 4, unbuilt.
