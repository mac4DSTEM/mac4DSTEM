...
**A — "a weird overlap from time to time"**, still open. Asked for more (which room, when); his own answer: "I think it happened in all many different runs but not sure, I can test more later." No fix attempted — nothing in `WorkspaceView.swift`/`ContentView.swift` uses `.zIndex`/`.offset` in a way that would explain a stray overlap, so this is not a quick static-read find; needs a screenshot or a screen recording when it recurs. Owner: catch it when it happens.
