#!/usr/bin/env python
"""export.py — the trained net to Core ML (.mlpackage, the shipping route since C7, 2026-09-08) and Core AI (.aimodel).
...
Core ML (detect variant, macOS 15 target): coremltools from the same net, float16, heatmap + peaks if
the converter takes them, heatmap only otherwise (recorded).
Core ML heatmap (C7, the SHIPPING asset): `disk-detector-heatmap-<size>.mlpackage`, float16 in and out,
the batch a flexible dimension 1...--batch-max (default --primary), the run's own spatial size, macOS 14
target (the app's floor) — no in-graph programme, the app peak-picks on the CPU (decisions.md 2026-09-08).
A several-shape package (spatial sizes --shapes, the owner's "tried first") is exported beside it and its
outcome recorded; the app uses the fixed-size one (windows above the size).
...
def export_coreml_heatmap(unet: nn.Module, size: int, out: Path, log: dict, batch: int = 32, batch_max: int = 64,
                          sizes: list | None = None):
    """The shipping asset (C7): heatmap only, float16 I/O, flexible batch (or, with `sizes`, an enumerated
    set of (1|batch, 3, s, s) shapes), macOS 14 target. Records the package's I/O in `log`."""
    import coremltools as ct
    example = torch.rand(batch, 3, size, size)
    traced = torch.jit.trace(HeatmapOnly(unet).eval(), example)
    if sizes:
        shapes = [(b, 3, s, s) for s in sizes for b in (1, batch)]
        shape = ct.EnumeratedShapes(shapes=shapes, default=(batch, 3, size, size))
    else:
        shape = ct.Shape(shape=(ct.RangeDim(lower_bound=1, upper_bound=batch_max, default=batch), 3, size, size))
    m = ct.convert(traced, inputs=[ct.TensorType(name="x", shape=shape, dtype=np.float16)],
                   outputs=[ct.TensorType(name="heatmap", dtype=np.float16)],
                   minimum_deployment_target=ct.target.macOS14, compute_precision=ct.precision.FLOAT16, convert_to="mlprogram")
    m.short_description = f"mac4DSTEM learned disk-candidate heatmap, {size} px, float16 (tools/disk-detector, C7 2026-09-08)"
    m.user_defined_metadata["size"] = str(size)
    m.user_defined_metadata["shapes"] = "enumerated " + ",".join(map(str, sizes)) if sizes else f"batch 1...{batch_max} (default {batch})"
    m.save(str(out))
    log["coreml_heatmap_io"] = f"x [B,3,{size},{size}] float16 -> heatmap [B,1,{size},{size}] float16; " + m.user_defined_metadata["shapes"]
    return out
...
def main():
    ...
    ap.add_argument("--skip-coreml-heatmap", action="store_true", help="skip the shipping Core ML heatmap package (C7)")
    ap.add_argument("--skip-coreai", action="store_true", help="skip every Core AI asset (re-export one Core ML route alone)")
    ap.add_argument("--batch-max", type=int, default=64, help="upper bound of the flexible batch of the Core ML heatmap package")
    ap.add_argument("--shapes", type=int, nargs="*", default=[256, 512], help="spatial sizes of the several-shape Core ML attempt (empty: skip)")
    a = ap.parse_args()
    ...
    # an existing export.json is the base, so one route can be re-exported alone (C7: the Core ML heatmap
    # package added to the 2026-09-08 run without redoing its Core AI assets); its dicts are kept
    prev = json.load(open(out / "export.json")) if (out / "export.json").exists() else {}
    meta = dict(prev, run=a.run, batch=a.primary, threshold=a.threshold, top_k=a.top_k, size=size, params=sum(p.numel() for p in model.parameters()),
                date=time.strftime("%Y-%m-%d %H:%M"), macos=platform.mac_ver()[0], torch=torch.__version__, coreai_torch=coreai_torch.__version__,
                coremltools=coremltools.__version__, coreai=prev.get("coreai"), coreai_batches=prev.get("coreai_batches", {}),
                sha256=dict(prev.get("sha256", {})), failures=dict(prev.get("failures", {})))
    try:
        import coreai_opt; meta["coreai_opt"] = coreai_opt.__version__
    except Exception: pass
    meta["coreai_assets"] = dict(prev.get("coreai_assets", {}))
    for b in ([] if a.skip_coreai else a.batches):
    ...
    if not a.skip_stateful and not a.skip_coreai:
    ...
    if not a.skip_coreml_heatmap:
        p = out / f"disk-detector-heatmap-{size}.mlpackage"; log = {}
        try:
            export_coreml_heatmap(model, size, p, log, batch=a.primary, batch_max=a.batch_max)
            meta["coreml_heatmap"] = str(p); meta["coreml_heatmap_io"] = log["coreml_heatmap_io"]; meta["sha256"][p.name] = sha256_tree(str(p))
            print(f"Core ML heatmap: {p} ({log['coreml_heatmap_io']}) sha256 {meta['sha256'][p.name][:16]}")
        except Exception as e:
            meta["failures"]["coreml_heatmap"] = f"{type(e).__name__}: {e}"[:600]; print(f"Core ML heatmap FAILED: {type(e).__name__}: {e}"); traceback.print_exc()
        if a.shapes:
            p = out / f"disk-detector-heatmap-shapes.mlpackage"; log = {}
            try:
                export_coreml_heatmap(model, size, p, log, batch=a.primary, sizes=a.shapes)
                meta["coreml_heatmap_shapes"] = str(p); meta["coreml_heatmap_shapes_io"] = log["coreml_heatmap_io"]; meta["sha256"][p.name] = sha256_tree(str(p))
                print(f"Core ML several-shape: {p} ({log['coreml_heatmap_io']})")
            except Exception as e:
                meta["failures"]["coreml_heatmap_shapes"] = f"{type(e).__name__}: {e}"[:600]; print(f"Core ML several-shape FAILED: {type(e).__name__}: {e}"); traceback.print_exc()
