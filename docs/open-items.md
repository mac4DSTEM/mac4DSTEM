...
**A — the centre header reads as empty.** On the calibrated demo Prepare has no next verb, so the row shows only the breadcrumb at the left and Save to Session / Reveal at the right with the flexible gap between; with no dataset it shows "mac4DSTEM" and a disabled Reveal. Owner wants the file name, Save to Session, Reveal in Finder and the current run there. **Owner decision owed** (frozen shell): the four header states proposed 2026-09-22 — no dataset: no row; open step: today's row; room complete: state chip in the gap, verb = next room; run in flight: run name · progress · ETA in the gap, Cancel in the verb's place (then the infobar's own bar goes, one bar not two).
