...
| UI2 — every surface migrated | done 2026-09-04; owner-driven twice, findings fixed; **Gate D refuter ran** | `UI2/` is the whole app in SwiftUI: 20 files, zero references to any view under `UI/`. Contract in `architecture.md` "The UI2 contract", now pinned by an `inventory` grep (`HSplitView`/`NSSplitView`/`import AppKit` in `UI2/`, mutation-tested both ways). Shape and drive decisions in `decisions.md` (2026-09-04). The launch crash is fixed (`UI2PaneSplit`) and gated, but **its mechanism is refuted, not established** — the refuter killed the explanation with `UI/ContentView.swift:539`, and `open-items.md` names the two probes that would decide it. The owner's drive produced eight findings, all fixed: run action to the toolbar's trailing edge with a Cancel-only busy state; segmented pictogram pickers; an accent outline on the pane Imaging's Direction drives; no scan marker in Results; "Save to Results" where the result is made; the folder icon became the dataset menu the migration dropped; Dataset + Session sections fill the sidebar's foot and promote the sidecar warnings out of Info. `UI/` is untouched and still the default; `--ui2` selects the rebuild |
